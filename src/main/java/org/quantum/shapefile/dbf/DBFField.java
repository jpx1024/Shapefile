...
public class DBFField implements Serializable {
    ...
    public Object readValue(final DataInputStream dataInputStream, Charset charset) throws IOException {
    ...
    private long readTimeInMillis(final byte[] bytes) {
    ...
    private float readFloat(final byte[] bytes) {
        final int index = trimSpaces(bytes);
        final int length = bytes.length - index;
        if (length == 0 || bytes[index] == '?') {
            return 0.0F;
        ...
        String str = new String(bytes, index, length);
